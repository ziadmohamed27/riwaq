...
export default function CheckoutSuccessPage({ searchParams }: PageProps) {
  ...
  // لو لا يوجد orderNumber → redirect للرئيسية
    ...
    <div dir="rtl" className="flex min-h-screen items-center justify-center bg-stone-50 px-4">
      <div className="w-full max-w-md text-center">
        ...
        <div className="mt-6 rounded-2xl border border-stone-200 bg-white px-6 py-5">
          <p className="text-xs text-stone-400 mb-1">رقم طلبك</p>
          <p className="text-xl font-mono font-bold tracking-wide text-stone-800">
            {orderNumber}
          </p>
        ...
        {/* ── الخطوات القادمة ───────────────────────────────────────────── */}
        <div className="mt-6 rounded-2xl bg-amber-50 border border-amber-100 px-5 py-4 text-start space-y-2">
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
          <Link
            href="/account/orders"
            className="flex items-center justify-center gap-2 rounded-xl bg-amber-500 px-6 py-3 text-sm font-semibold text-white hover:bg-amber-600 transition shadow-sm"
          >
          ...
          <Link
            href="/marketplace"
            className="flex items-center justify-center gap-2 rounded-xl border border-stone-200 bg-white px-6 py-3 text-sm font-medium text-stone-700 hover:bg-stone-50 transition"
          >
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
